...
class TestExtractDenylist:
    ...
    def test_nested_same_key(self):
        cfg = Config(
            raw={"deny_lists": {"organizations": {"organizations": ["Acme", "Globex"]}}}
        )
        assert _extract_denylist(cfg, "organizations") == ["Acme", "Globex"]

    def test_first_list_in_dict(self):
        cfg = Config(
            raw={"deny_lists": {"organizations": {"items": ["Acme"], "version": 1}}}
        )
...
class TestDetect:
    ...
    def test_singleton_from_config(self):
        cfg = Config(raw={"deny_lists": {"organization_singleton": ["Acme Corp"]}})
