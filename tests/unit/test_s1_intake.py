...
class TestIntake:
    ...
    def test_missing_file(self, tmp_path: Path):
        with pytest.raises(IntakeError, match="introuvable"):
            intake(tmp_path / "absent.txt")

    def test_empty_file(self, tmp_path: Path):
        f = tmp_path / "empty.txt"
        f.write_text("", encoding="utf-8")
        with pytest.raises(IntakeError, match="vide"):
            intake(f)

    def test_too_large(self, tmp_path: Path):
        f = tmp_path / "big.txt"
        f.write_text("x" * 200)
        with pytest.raises(IntakeError, match="trop gros"):
            intake(f, max_size_bytes=100)

    def test_not_a_file(self, tmp_path: Path):
        with pytest.raises(IntakeError, match="Pas un fichier"):
            intake(tmp_path)
