...
class TestMaskBasic:
    def test_single_person(self):
        ...
        assert out.anonymized_text == "Hello [PERSON_1]."
        assert out.original_text == text
        assert vault.get_original("[PERSON_1]") == "Alice"
    ...
    def test_multiple_types(self):
        ...
        vault = Vault(password=VALID_PWD)
        out = mask(_resolved(text, spans), vault)
        assert "[PERSON_1]" in out.anonymized_text
        assert "[ORG_1]" in out.anonymized_text
    ...
    def test_same_value_same_placeholder(self):
        ...
        assert out.anonymized_text.count("[PERSON_1]") == 3
        assert len(vault) == 1
    ...
    def test_singleton_no_index(self):
        ...
        vault = Vault(password=VALID_PWD)
        out = mask(_resolved(text, spans), vault)
        assert "[ORG]" in out.anonymized_text
        assert "[ORG_1]" not in out.anonymized_text  # pas d'index pour singleton
        assert out.anonymized_text.count("[ORG]") == 2
