...
class TestFiscalFr:
    ...
    def test_vat_fr(self):
        assert len(recognize_vat_fr("TVA FR66529268393")) == 1
        ...
        assert len(recognize_vat_fr("fr66529268393")) == 0

    def test_aggregator(self):
        text = "SIRET 380.590.489.00015 - SIREN 440 236 453 - TVA FR66529268393"
...
class TestIbanBic:
    def test_iban_checksum_valid(self):
        assert validate_iban_checksum("FR7617629000010011369280060")
    ...
    def test_iban_match(self):
        spans = recognize_iban("IBAN FR7617629000010011369280060 BIC COBA")
        assert len(spans) == 1
        assert spans[0].entity_type == "IBAN"

    def test_iban_glued(self):
        # IBAN colle au mot precedent
        spans = recognize_iban("IBANFR7617629000010011369280060 BIC")
        assert len(spans) == 1
    ...
    def test_bic_requires_context(self):
        ...
        assert len(recognize_bic("BIC COBAFRPXXXX")) == 1
        assert len(recognize_bic("SWIFT COBAFRPXXXX")) == 1
...
class TestAddressFr:
    def test_postal_city(self):
        spans = recognize_postal_city_fr("Livraison 49124 ST SPRINGFIELD D'ANJOU")
        assert len(spans) == 1
        assert "SPRINGFIELD" in spans[0].text

    def test_postal_city_glued(self):
        # Texte parasite colle
        spans = recognize_postal_city_fr("49124STSPRINGFIELDDANJOU")
        assert len(spans) >= 1

    def test_zone(self):
        spans = recognize_zone_fr("Adresse PARC ACTIVITE ANGERS/ST SPRINGFIELD")
        assert len(spans) == 1
...
class TestNamePatterns:
    ...
    def test_civilite(self):
        spans = recognize_civilite("Bonjour M. DOE Jane")
        assert len(spans) == 1
